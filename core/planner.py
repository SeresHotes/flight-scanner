...
# Жёсткий серверный потолок числа цепочек. Прод — burstable VM (4 ГБ): безлимитный
# перебор (max_results=None) на плотном графе с двумя «any» строит сотни тысяч
# Itinerary → отдаёт сотни МБ JSON, вешает браузер и почти достаёт до OOM. Воркер
# ЖЁСТКО зажимает max_results этим числом независимо от клиента (старый закешированный
# фронт слал None). Показ на фронте всё равно страницами.
PLAN_HARD_MAX_RESULTS = 1000


def clamp_max_results(n: Optional[int]) -> int:
    """Зажимает число цепочек жёстким потолком: None (старый клиент/прямой API) или
    значение больше PLAN_HARD_MAX_RESULTS → потолок; иначе не меньше 1."""
    if n is None or n > PLAN_HARD_MAX_RESULTS:
        return PLAN_HARD_MAX_RESULTS
    return max(1, n)
