...
import aggregate_flights as agg
...
def main():
    ...
    # значения по умолчанию для arrive-by: за день до события
    if a.arrive_by is None and a.event_date:
        a.arrive_by = (datetime.strptime(a.event_date, "%Y-%m-%d") - timedelta(days=1)).strftime("%Y-%m-%d")

    network = agg.load_airport_network(a.network)
    city_info = make_city_lookup(network)

    hub_ci = city_info(a.hub)
    region_label = a.region_label or hub_ci["city"]
    region_flag = a.region_flag or hub_ci["flag"]

    default_exclude = {"MOW", "SVO", "DME", "VKO", "ZIA", "LED",
                       "ICN", "SEL", "GMP", "PUS", "CJU"}
    exclude = set(a.exclude) if a.exclude is not None else default_exclude
    exclude |= {a.origin, a.hub}

    class Cfg: pass
    cfg = Cfg()
    cfg.exclude = exclude
    cfg.min_stay, cfg.max_stay = a.min_stay, a.max_stay
    cfg.stop_min, cfg.stop_max = a.stopover_days
    cfg.arrive_by = a.arrive_by
    cfg.event_date = a.event_date or None
    # переходы между близкими аэропортами в городе остановки
    cfg.airport_network = None if a.no_airport_transfers else network
    cfg.airport_distance = a.airport_distance
    cfg.same_country = not a.cross_country_transfers
    # для односторонних «обратно»: вылет не раньше, чем прилёт + мин. пребывание
    cfg.return_from = None
    if a.arrive_by:
        cfg.return_from = (datetime.strptime(a.arrive_by, "%Y-%m-%d")
                           + timedelta(days=a.min_stay)).strftime("%Y-%m-%d")

    b = Builder(cfg, city_info)

    plain = load(a.plain)
    plain_out = plain["leg1_flights"]   # ORIGIN -> HUB
    plain_ret = plain["leg2_flights"]   # HUB -> ORIGIN
    there_merged = load_merged([a.there, a.there_indirect]) if a.there else None
    back_merged = load_merged([a.back, a.back_indirect]) if a.back else None
    ...
    max_korea = a.max_stay
    if there and back:
        max_korea = max(o["hub_ord"] for o in back) - min(o["hub_ord"] for o in there)

    title = a.title
    if not title:
        title = f"{a.origin_name.split(' (')[0]} → {region_label} ⇄ обратно"
        if a.event_name:
            title += f" · {a.event_name.split(' —')[0]}"

    event = None
    if a.event_date:
        event = {"name": a.event_name, "date": a.event_date, "venue": a.event_venue}
    ...
    out = {
        "meta": {
            ...
            "title": title,
            "currency": a.currency,
            "origin": a.origin_name, "origin_code": a.origin,
            "destination": a.hub_name, "destination_code": a.hub,
            "region_label": region_label, "region_flag": region_flag,
            "event": event,
            "min_stay": a.min_stay,
            "max_korea_days": max(a.min_stay, max_korea),
            "stopover_days_range": list(a.stopover_days),
            "max_stop_days": a.stopover_days[1],
    ...
    print(f"  Городов: туда {len(there_cities)} / обратно {len(back_cities)} | "
