...
### Планировщик цепочек A → B → C (страница `/planner`)

Отдельная страница поверх того же ядра: маршрут — линейная цепочка остановок, каждая
из которых — НАБОР городов-кандидатов (`kind:'cities'`) или «любой» (`kind:'any'`,
только в середине). Окно дат «когда ОК быть здесь» — только у промежуточных остановок;
у концов выводится из соседей. Фильтры/сортировка — клиентские (как в основном поиске).

**Бэк реализован** (`core/planner.py` + эндпоинты в `api/main.py`, воркер `run_plan_collection`):
- `POST /api/plan/estimate` — оценка `{requests, seconds, legs}`. Сбор экономный: на каждый
  переход «якорим» сторону с МЕНЬШИМ числом городов и запрашиваем через неё «все направления»
  (1 запрос/день на якорный город), другую сторону фильтруем по выбранным городам — конкретные
  города не дороже «любого». Формула совпадает с клиентской `frontend/src/planner/estimate.ts`.
- `POST /api/plan/gather` — заводит фоновую джобу (тот же `jobs` + `ThreadPoolExecutor`),
  предохранитель `MAX_REQUESTS=200`. Собирает плечи через `core.collector.collect_leg_data`
  (allow_indirect), стыкует цепочки DFS (рейс подходит, если вылетает не раньше прилёта и
  приземляется в разрешённом для следующей остановки городе), переиспользует
  `trip_builder.make_segment`/`make_city_lookup`. Котировки → SQLite + Parquet-озеро.
- `GET /api/plan/jobs/{id}` — прогресс, по завершении — массив `Itinerary` (контракт
  `frontend/src/planner/types.ts`). Клиент (`planner/api.ts`) поллит и рендерит карточки.

Фронт отвязан от мока: оценка — `planner/estimate.ts` (мгновенная, клиентская), сбор —
`planner/api.ts` (реальный gather + polling). `planner/mock.ts` удалён.
